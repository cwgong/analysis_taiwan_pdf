...
def make_pic(indir, filename, out_dir):  # 使用ghostscript 將pdf轉為每頁圖片
    ...
    pdf_route = '{}/{}'.format(indir, filename)
...
def preprocess(src):  # 影像之前處理
    gray = cv2.cvtColor(src, cv2.COLOR_BGR2GRAY)#转换为灰度图像
    outs = cv2.adaptiveThreshold(255 - gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 15, -2)#自适应阈值处理该图像
...
def Form_DCT(indir, file_name, origin_img_dir):  # 圖片表格表格偵測多執行序
    ...
    img_source = '{}/{}/'.format(origin_img_dir, file_name.split('.')[0])#取到通过gs转换出来的每一张图片
...
def htplusimage(pdf_data_path, pdf):  # 內文偵測
    ...
    if not document.is_extractable:
        ...
        for idx, pagess in enumerate(all_record_txt):
            ...
            for items in pagess:
                if (items.bbox[1] > big_tail_L[idx] and page_rec_h[idx] - items.bbox[3] > head[idx]) or abs(
...
# -------------------------------------------------
def horizonmerge_find_caption(txt_lst, val_txt_lst, img1, mask_with_txt, title, mask, boxes, flags, title_box, tail,
                              tail_box, all_txt_after_filt, all_txt_num, every_page_min_height, w, h, img3):
    # print('pre tail:',tail)
    ...
    for idx, tailbox in enumerate(tail_box):
        ...
        for idxs, txt_box in enumerate(new_filt_box):
            if (tail_box[idx] == None):
                continue
            if ((tailbox[0] <= txt_box[0]) and (tailbox[2] >= txt_box[2]) and (tailbox[1] <= txt_box[1]) and (
                    tailbox[3] >= txt_box[3])):
...
# ------------------------------------------------
def correct_wrongimg_to_form(boxes, flags, big_lst_name, title, txt_lst_se, val_txt_lst_se, w, h, img, all_dir, name,
                             tmp):
    for idxs, box in enumerate(boxes):
        ...
        if (flags[idxs] == 0 or flags[idxs] == 1):
            ...
            for idx, txt_box in enumerate(txt_lst_se):
                if min(txt_box[2] * w, box[2]) < max(txt_box[0] * w, box[0]) or min(txt_box[1] * h, box[3]) < max(
                        txt_box[3] * h, box[1]):
                    continue
                else:
...
def distingish_form_2column(new_filt_box, boxes, flags, title, title_box, big_lst_name):
    ...
    for idxs, box in enumerate(boxes):
        if (flags[idxs] == 2):
            txt_area = 0
            for idx, txt_box in enumerate(new_filt_box):
                if ((box[0] <= txt_box[0]) and (box[2] >= txt_box[2]) and (box[1] <= txt_box[1]) and (
                        box[3] >= txt_box[3])):
...
# -------------------------------------------------
def resize_output_form(boxes, flags, title, big_lst_name, img, w, h, name, tmp, all_dir, txt_lst_se, val_txt_lst_se,
                       new_filt_box, new_filt_txts, img3):
    all_form_content = []
    boxes_new = boxes[:]
    # print('boxes',boxes)
    a = 0
    for idx, box in enumerate(boxes):
        ...
        if num_form > 0:
            ...
            for idxs, form_box in enumerate(num_form_box):
                ...
                if idxs == 0:  # 第一個表格
                    # print('1')
                    boxes_new[idx + a] = [form_box[0] + box[0] - 3, form_box[1] + box[1], form_box[2] + box[0] + 3,
                                          form_box[3] + box[1]]
                    new_form_box.append(boxes_new[idx + a])
                    form_name = big_lst_name[idx + a]
                    all_form_content = form_output(all_form_content, boxes_new[idx + a], form_mask, form_name,
                                                   txt_lst_se, val_txt_lst_se, w, h, name, img)
                    # print('idx_num1',idx_num)
                else:  # 第二個以上表格
                    # print('2')
                    print('double form seperate')
                    cv2.imwrite('{}.jpg'.format(big_lst_name[idx + a].replace('FORM', 'IMAGE')), mask)
                    second_box = [form_box[0] + box[0] - 3, form_box[1] + box[1], form_box[2] + box[0] + 3,
                                  form_box[3] + box[1]]
                    new_form_box.append(second_box)
                    boxes_new.insert(idx + a + 1, second_box)
                    flags.insert(idx + a + 1, 2)
                    # print('idx_num2',idx_num)
                    big_lst_name.insert(idx + a + 1,
                                        "{}/{}/{}_{}_{}.jpg".format(all_dir[0], name, name, tmp, idx_num + 1))
                    form_name = "{}/{}/{}_{}_{}.jpg".format(all_dir[0], name, name, tmp, idx_num + 1)
                    all_form_content = form_output(all_form_content, second_box, form_mask, form_name, txt_lst_se,
                                                   val_txt_lst_se, w, h, name, img)
                    title.insert(idx + a + 1, '')
                    a += 1
                    # idx_num+=1
            new_filt_box, new_filt_txts = restore_txt(boxes_old, new_form_box, txt_lst_se, val_txt_lst_se, new_filt_box,
                                                      new_filt_txts, w, h, img3)
            ...
            if len(new_form_box) == 1:
                if (txt[1] * h <= new_form_box[0][1]):
                    if (re.search(r'(^单位.)', val_txt_lst_se[idxx])):
                        continue
                    # print(val_txt_lst_se[idxx])
                    new_filt_box, new_filt_txts = get_back_txt(idxx, txt_lst_se, val_txt_lst_se, new_filt_box,
                                                               new_filt_txts, w, h)
...
def form_output(all_form_content, form_box, form_mask, form_name, txt_lst_se, val_txt_lst_se, w, h, name, img):
    ...
    for idxs, formboxes in enumerate(formtxtboxes):
        ...
        for idxs1, formboxes1 in enumerate(formtxtboxes):
            ...
            if idxs1 == idxs:
                continue
            if (min(formboxes[2], formboxes1[2]) < max(formboxes[0], formboxes1[0])) or (
                    min(formboxes[3], formboxes1[3]) < max(formboxes[1], formboxes1[1])):
                continue
            # print(idxs)
            # print(idxs1)
            if ((formboxes[2] - formboxes[0]) * (formboxes[3] - formboxes[1])) > (
                    (formboxes1[2] - formboxes1[0]) * (formboxes1[3] - formboxes1[1])):
    ...
    if len(formtxtboxes) > 1:
        ...
        for idxs, txtbox in enumerate(txt_lst_se):
            if (box[0] <= txtbox[0] * w and box[1] <= txtbox[3] * h) and (
                    (box[2] + 10) >= txtbox[2] * w and box[3] >= txtbox[1] * h):
        ...
        for idxs, txtbox in enumerate(formtxtbox):  # 文字塊
            ...
            for idxs1, formboxes in reversed(list(enumerate(formtxtboxes))):  # 方框
                # NO重疊
                if min(txtbox[2] * w - 10 * 0.41, formboxes[2]) < max(txtbox[0] * w + 10 * 0.41, formboxes[0]) or min(
                        txtbox[1] * h - 10 * 0.41, formboxes[3]) < max(txtbox[3] * h + 10 * 0.41, formboxes[1]):
                    continue
                if box_num != 0:
                    ifspace = re.search(r'(\S+\s+\S+)', formcontent[idxs])
                    if ifspace:
                        if min(txtbox[2] * w - 25 * 0.41, formboxes[2]) < max(txtbox[0] * w + 15 * 0.41,
                                                                              formboxes[0]) or min(
                                txtbox[1] * h - 10 * 0.41, formboxes[3]) < max(txtbox[3] * h + 10 * 0.41, formboxes[1]):
                            continue
                    else:
        ...
        with open("content/{}/{}.csv".format(name, imgfmname), 'w', newline='') as csvfile:
                ...
                if len(thisformbox) > 1:
                                ...
                                if abs(formboxes[2] - nowformright) > 20 * 0.41:
                                    for idxs1, formboxes1 in enumerate(formtxtboxes):
                                        if abs(formboxes[2] - formboxes1[0]) < 20 * 0.41:
                                            if ((formboxes[1] + formboxes[3]) / 2) > formboxes1[1] and (
                                                    (formboxes[1] + formboxes[3]) / 2) < formboxes1[3]:
                    ...
                    for idxs, formboxes in enumerate(thisformbox2):
                        ...
                        for idxs1, formboxes1 in enumerate(formlist):
                            if abs(formboxes[0] - formboxes1[0]) < 5 * 0.41 and abs(
                                    formboxes[2] - formboxes1[2]) < 5 * 0.41:
                                continue

                            if formboxes[0] < ((formboxes1[0] + formboxes1[2]) / 2) and formboxes[2] > (
                                    (formboxes1[0] + formboxes1[2]) / 2):
                                if thisaddnum > 0:
        ...
        for idxs, txtbox in enumerate(txt_lst_se):
            if (box[0] <= txtbox[0] * w and box[1] <= txtbox[3] * h) and (
                    box[2] >= txtbox[2] * w and box[3] >= txtbox[1] * h):
                formtxtbox.append(txtbox)
                formcontent.append(val_txt_lst_se[idxs])
        # print(formcontent)
        if len(formtxtbox) > 5:
        ...
        if Lsentence > 0 and no_text == 0:
            ...
            for idxs, txtbox in enumerate(txt_lst_se):
                if (txtbox[0] * w) > box[0] and (txtbox[2] * w) < box[2] and (txtbox[1] * h) > box[1] and (
                        txtbox[3] * h) < box[3]:
                    ...
                    if titlesearch3:
                        continue
                    cv2.rectangle(nonlineformimg, (round(txtbox[0] * w) - box[0], round(txtbox[1] * h) - box[1]),
                                  (round(txtbox[2] * w) - box[0], round(txtbox[3] * h) - box[1]), (0, 0, 0), -2, 8)
...
def get_back_txt(idxx, txt_lst_se, val_txt_lst_se, new_filt_box, new_filt_txts, w, h):
    new_filt_box.append([int(txt_lst_se[idxx][0] * w), int(txt_lst_se[idxx][3] * h), int(txt_lst_se[idxx][2] * w),
                         int(txt_lst_se[idxx][1] * h)])
...
def multi_draw(page_num, form_lst, f_img_lst, txt_lst, img_lst, val_txt_lst, origin_img_dir, name, all_dir, show_all,
               show_caption, thresh_h, thresh_t, txt_lst_se, val_txt_lst_se, head_txt_box, tail_txt_box, head_txt_page,
               tail_txt_page):  # 對所有物件 圖表內文做分析及篩選
    ...
    for idxs, txt_box in enumerate(txt_lst_se):
        cv2.rectangle(img1, (round(txt_box[0] * w), round(txt_box[3] * h)),
                      (round(txt_box[2] * w), round(txt_box[1] * h)), (255, 0, 255), 3, 8)
    ...
    mask, mask_big, big_lst_name, boxes, flags \
        = merge_horizontal_form_image(h, w, img_lst, f_img_lst, form_lst, all_dir[0], all_dir[2], name, tmp, thresh_h,
                                      img3)
    ...
    img1, mask_with_txt, title, title_box, tail, tail_box, all_txt_after_filt, all_txt_num \
        = horizonmerge_find_caption(txt_lst, val_txt_lst, img1, mask_with_txt, title, mask, boxes, flags, title_box,
                                    tail, tail_box, all_txt_after_filt, all_txt_num, every_page_min_height, w, h, img3)
    ...
    if (page_num == 0):
        ...
        boxes, flags, title, title_box, big_lst_name, new_filt_box, new_filt_txts \
            = distingish_nonlineform_2column(boxes, flags, title, title_box, big_lst_name, new_filt_box, new_filt_txts,
                                             img1)
    ...
    # -----------------------------------
    boxes, flags, big_lst_name, title = correct_wrongimg_to_form(boxes, flags, big_lst_name, title, txt_lst_se,
                                                                 val_txt_lst_se, w, h, img, all_dir, name, tmp)

    # -----------------------------------
    # delete first page wrong img and form(第一頁比較亂)
    if (page_num == 0):
        boxes, flags, title, title_box, big_lst_name = del_firstpage_wrong_imgform(boxes, flags, title, title_box,
                                                                                   big_lst_name, h)
    ...
    boxes, flags, title, big_lst_name, new_filt_box, new_filt_txts, all_form_content \
        = resize_output_form(boxes, flags, title, big_lst_name, img, w, h, name, tmp, all_dir, txt_lst_se,
                             val_txt_lst_se, new_filt_box, new_filt_txts, img3)
    # if page_num==8:
    # print('----------------')
    ...
    all_word_txt_box = []
    now_word_txt = []
    with open("clean_TXT/{}/{}_{}.txt".format(name, name, tmp), "w") as text_file2:
        max_right_box = 0
        min_left_box = w
        word_high = []
        word_long = []
        ...
        if len(word_right_box) > 0:
            for idx, wordright in enumerate(word_right_box):
                ...
                for idxs, wordlong_1 in enumerate(wordrightidx):
                    if abs(wordrightidx[idxs] - wordright) < 41:
                        a = wordright_num[idxs]
                        a += 1
                        wordright_num[idxs] = a
                        wordmatch = True
                        break
                if wordmatch == True:
                    continue
        ...
        for idx, txt in enumerate(new_filt_txts):
            ...
            if len(xxxx) > 0:
                # print('k')
                if xxxx[0] == '图' and (new_filt_box[idx][2] - new_filt_box[idx][0]) < 0.6 * w and (
                        new_filt_box[idx][3] - new_filt_box[idx][1]) < 0.1 * h:
                    # print('j')
                    continue
                ...
                if answernn:
                    ...
                    if (idx == (len(new_filt_box) - 1)):
                        ...
                        txts_box = str()
                        continue
                    if abs(new_filt_box[idx][2] - max_right_box) > 85:
                        txts_box = txts_box + xxxx
                        # print(txts_box)
                        # print('222222222')
                        # print('txt')
                    ...
                    # print(xxxx)
                    # print(new_filt_box[idx])
                    answernn2 = re.search(r'^\s{2,}', new_filt_txts[idx + 1])
                    if ((abs(new_filt_box[idx][0] - new_filt_box[idx + 1][0])) > (48)) or (answernn2):
                        txts_box = txts_box + xxxx
                        # print(txts_box)
                        # print('3333333333')
                        # print('txt')
                    ...
                    if ((new_filt_box[idx][3] - new_filt_box[idx][1]) > 73):
                        txts_box = txts_box + xxxx
                        # print(txts_box)
                        # print('4444444444')
                        # print('txt')
                    ...
                    # print('h')
                    if (new_filt_box[idx][3] - new_filt_box[idx][1]) > 85:
                        # print('a')
                        # print(xxxx)
                        # print('55555555555')
                        # print('txt')
                        # print((new_filt_box[idx][2]-new_filt_box[idx][0]))
                        text_file2.write(xxxx)
                        now_word_txt.append(xxxx)
                        text_file2.write('\n')
                        txts_box = str()
                        continue
                    if ((new_filt_box[idx][2] - new_filt_box[idx][0]) > (sen_long * 0.7)) and (
                            idx < (len(new_filt_box) - 1)):
                        if (new_filt_box[idx + 1][1] - new_filt_box[idx][3]) < (
                                new_filt_box[idx][3] - new_filt_box[idx][1]) * 3:
                            if abs(new_filt_box[idx][2] - max_right_box) < w * 0.035:
                                # print('b')
                                # print('------------------')
                                # print(xxxx)
                                txts_box = txts_box + xxxx
                                ...
                                if txts_box != str():
                                    ...
                                    text_file2.write('\n')
                                    txts_box = str()
                                    all_word_txt_box.append(new_filt_box[idx - 1][1])
                                # print(xxxx)
                                # print('6666666666')
                                # print('txt')
                                text_file2.write(xxxx)
        ...
        if len(all_word_txt_box) == 0 or len(now_word_txt) == 0:
            for idx, box in enumerate(boxes_out):
                if flags[new_idx[idx]] == 0 or flags[new_idx[idx]] == 1:
                    Object = {"type": "graph", "content": big_lst_name[new_idx[idx]], "header": head_txt,
                              "footer": tail_txt, "page_num": page_num + 1}
                    ...
                    Object = {"type": "table", "content": all_form_content[new_idx[idx] - img_add], "header": head_txt,
                              "footer": tail_txt, "page_num": page_num + 1}
                    list_obj.write(json.dumps(Object, indent=4, sort_keys=True, ensure_ascii=False) + '\n')
        elif len(boxes) == 0:
            for idx, txt in enumerate(now_word_txt):
                Object = {"type": "text", "content": txt, "header": head_txt, "footer": tail_txt,
                          "page_num": page_num + 1}
                list_obj.write(json.dumps(Object, indent=4, sort_keys=True, ensure_ascii=False) + '\n')
        else:
            used_idxs = []
                ...
                for idxs, box2 in enumerate(all_word_txt_box):
                    ...
                    if box[1] < box2:
                        if flags[new_idx[idx]] == 0 or flags[new_idx[idx]] == 1:
                            ...
                            # print(page_num)
                            # print('----------')
                            Object = {"type": "table", "content": all_form_content[new_idx[idx] - img_add],
                                      "header": head_txt, "footer": tail_txt, "page_num": page_num + 1}
                            list_obj.write(json.dumps(Object, indent=4, sort_keys=True, ensure_ascii=False) + '\n')
                        break
                    else:
                        # print(idxs)
                        # print('--------------------',now_word_txt[idxs])
                        Object = {"type": "text", "content": now_word_txt[idxs], "header": head_txt, "footer": tail_txt,
                                  "page_num": page_num + 1}
                        list_obj.write(json.dumps(Object, indent=4, sort_keys=True, ensure_ascii=False) + '\n')
    ...
    return title, big_lst_name, matchtitle, matchtitlebox, boxes, page_num_box, title_page_num, h, w
...
def dist_2col(src):
    ...
    if len(word_w) > 1:
        if ((word_w[0] > w * 0.4 and word_w[0] < w * 0.8) and (word_w[0] > word_w[1] * 1.25) and (
                word_w[1] > w * 0.05)):
...
def dist_nonline_2col(src_im):
    ...
    if len(word_w) >= 2:
        if (word_w[0] > w / 2 and word_w[0] < w * 0.8) or len(word_h) <= 2:
            if (len(word_h) <= 2):
                return True, right, line_pos
            if ((word_h[0] - 5) > word_h[1] and (word_h[0] - 5) > hmean and (word_h[1] - 5) > hmean) or len(
                    word_h) <= 2:
                return True, right, line_pos
            else:
                return False, right, line_pos
...
def drawbox_v1(form_lst, f_img_lst, txt_lst, img_lst, val_txt, origin_img_dir, name, all_dir, show_all, show_caption,
               thresh_h, thresh_t, txt_lst_se, val_txt_se, head_txt, tail_txt, head_txt_page,
               tail_txt_page):  # 多執行序執行統整
    ...
    for page_num in range(len(form_lst)):
        res1 = multi_draw(page_num, form_lst[page_num], f_img_lst[page_num], txt_lst[page_num], img_lst[page_num],
                          val_txt[page_num], origin_img_dir, name, all_dir, show_all, show_caption, thresh_h[page_num],
                          thresh_t[page_num], txt_lst_se[page_num], val_txt_se[page_num], head_txt, tail_txt,
                          head_txt_page, tail_txt_page)
        lst.append(res1)
    lst_name = []
    lst_caption = []
    titlecontent = []
    ...
    page_w = 0
    for i in lst:
        lst_caption.extend(i[0])
        lst_name.extend(i[1])
        titlecontent.extend(i[2])
        titlebox.extend(i[3])
        imgboxes.extend(i[4])
        pagenum.extend(i[5])
        titlepagenum.extend(i[6])
        page_h = i[7]
        page_w = i[8]
    # --------------------------------------------------------------
                ...
                if (pagenum[idx] - 1) > 0:
                    for idxs, txtbox in enumerate(titlebox):
                        if (pagenum[idx] - 1) == titlepagenum[idxs]:
                            if ((imgboxes[idx][2]) > (txtbox[0] * page_w)) and (
                                    (imgboxes[idx][0]) < (txtbox[2] * page_w)):
    ...
    with open("{}/{}.txt".format(dir5, name), "w") as all_text_file:
        for num in range(page_num):
            ...
            with open("{}/{}/{}_{}.txt".format(dir2, name, name, tmp), "r") as text_file:
                ...
                text_file.close()
        all_text_file.close()
    with open("output_json/{}.json".format(name), "w") as all_text_file:
        for num in range(page_num):
            with open("output_json/{}/{}.json".format(name, num + 1), "r") as text_file:
                all_text_file.write(text_file.read())
                text_file.close
        all_text_file.close()
...
# --------------
def download_pdf(file_path, file_name):
    r = requests.get(file_path, stream=True)
    # print(r.status_code,'-----------------------')
    if r.status_code == requests.codes.ok:
        with open('./pdf_data/{}'.format(file_name), 'wb') as pdf:
    ...
    if download_pdf(file_path, file_name):
        ...
        if len(val1) != page_all:
            ...
            drawbox_v1(val1, val2, val3, val4, val_txt, origin_img_dir, name, all_dir, show_all, show_caption, thresh_h,
                       thresh_t, val5, val_txt_se, head_txt, tail_txt, head_txt_page, tail_txt_page)
...
if __name__ == '__main__':
    ...
    # main('n1.pdf',indir,origin_img_dir,all_dir,show_all,show_whole_txt,show_caption)
    # for i in range(14,12,-1):
    #     main('{}.pdf'.format(i),indir,origin_img_dir,all_dir,show_all,show_whole_txt,show_caption)
